...
document.getElementById("duolingo-link-stage-3").addEventListener('click', () => { 
...
const popupWindow = chrome.windows.getCurrent().then(function(response) {
    ...
    window.moveTo(parseInt(window.screen.width/2 - 250), parseInt(window.screen.height/2 - 250));
...
async function getDuolingoData() {
    ...
    const response = await fetch("https://www.duolingo.com/api/1/users/show?username=" + myUsername.username);
