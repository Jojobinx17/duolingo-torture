...
// The first few versions of this extension were entirely created on my school chromebook with its default text editor! It had syntax highlighting, but not much else. Made debugging a little tricky, lol.
  ...
  fetch("https://www.duolingo.com/api/1/users/show?username=" + usernameToCheck)
    ...
    .then((data) => {
      ...
      chrome.storage.local
        .set({ username: usernameToCheck })
        .then(getStoredUsername());
    })
    .catch((error) => {
...
async function showUsernameError(error) {
  if (error == "err-not-found") {
    document.getElementById("current-username").innerHTML =
      "Username does not belong to a Duolingo account.";
  } else if (error == "err-auth") {
    lastEnteredUsername = "";
    document.getElementById("current-username").innerHTML =
      'Unable to access the Duolingo API. Please make sure that you are signed into <a style="font-size: 18" target="_blank" href="https://duolingo.com">duolingo.com</a> on this device.';
  } else {
    lastEnteredUsername = "";
    document.getElementById("current-username").innerHTML =
      "An error occured. Please make sure you are signed into duolingo.com, and you typed in the username correctly.";
  }
}
...
async function getStoredUsername() {
  ...
  if (
    ...
    storedUsername.username == undefined
  ) {
  ...
  for (let i = 0; i < languages.length; i++) {
    if (languages[i].current_learning == true) {
      await chrome.storage.local.set({
        language: languages[i].language_string,
      });
...
document
  .getElementById("close-diff-info-button")
  .addEventListener("click", () => {
...
function selectDifficulty(level, setting) {
  if (level == 0) {
    document
      .getElementById("diff-easy-button")
      .classList.remove("button-selected");
    document
      .getElementById("diff-normal-button")
      .classList.remove("button-selected");
    document
      .getElementById("diff-hard-button")
      .classList.remove("button-selected");
    document
      .getElementById("diff-duo-button")
      .classList.remove("button-selected");
    ...
    document
      .getElementById("diff-duo-button")
      .classList.remove("button-blocked");
    document.getElementById("difficulty-flavour-text").innerHTML =
  ...
  if (level == 3) {
    ...
    document.getElementById("diff-title").style.display = "block";
    document.getElementById("difficulty-flavour-text").innerHTML =
      "Difficulty set to Torture. Good luck!";
...
async function resetData() {
  ...
  await chrome.storage.local.set({ debugVerbosity: 0 });
...
async function initDebug() {
  chrome.storage.local.get(null).then(function (result) {
    ...
    document.getElementById("debug-verbosity-data").innerHTML =
      "Debug verbosity: " + savedData.debugVerbosity;
...
document
  .getElementById("debug-verbosity-button")
  .addEventListener("click", () => {
    let inputValue = parseInt(
      document.getElementById("debug-verbosity-input").value,
    );

    // I love ternary so much
    inputValue = inputValue < 0 ? 0 : inputValue > 1 ? 1 : inputValue;

    chrome.storage.local.set({ debugVerbosity: inputValue }).then(() => {
      chrome.storage.local.get(["debugVerbosity"]).then(function (response) {
        document.getElementById("debug-verbosity-data").innerHTML =
          "Debug verbosity: " + response.debugVerbosity;
      });
    });
  });
...
document
  .getElementById("d-edit-diff-reload-button")
  .addEventListener("click", () => {
    window.location.reload();
  });
...
document.getElementById("ignore-diff-button").addEventListener("click", () => {
  savedData.ignoreDifficulty =
    savedData.ignoreDifficulty == true ? false : true;

  chrome.storage.local
    .set({ ignoreDifficulty: savedData.ignoreDifficulty })
    .then(() => {
...
document.getElementById("set-time-button").addEventListener("click", () => {
  ...
  const unparsedDate = new Date(
    d.toString().split(":")[0].slice(0, -2) + inputValue,
  );
...
document.getElementById("set-time-2-button").addEventListener("click", () => {
  ...
  chrome.storage.local.set({ time2: parsedDate }).then(() => {
    document.getElementById("set-time-2-data").innerHTML =
      "First page load set to " +
      parsedDate +
      " (" +
      humanReadableDate(parsedDate) +
      ")";
  });
});
...
document.getElementById("set-time-3-button").addEventListener("click", () => {
  ...
  chrome.storage.local.set({ time3: parsedDate }).then(() => {
    document.getElementById("set-time-3-data").innerHTML =
      "Time of last popup has been set to " +
      parsedDate +
      " (" +
      humanReadableDate(parsedDate) +
      ")";
